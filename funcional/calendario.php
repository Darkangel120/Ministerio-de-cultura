...
// Verificar permisos - Funcionarios, Admin y Directores pueden acceder al calendario
...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'get_eventos') {
        ...
        exit();
    } elseif ($_GET['action'] === 'execute_event') {
        // Ejecutar actividad - cambiar estado a 'reportada'
        $pdo = conectarDB();
        $id = (int)$_GET['id'];

        $stmt = $pdo->prepare("UPDATE eventos SET estado_ejecucion = 'reportada' WHERE id = ? AND activo = 1");
        $result = $stmt->execute([$id]);

        if ($result) {
            echo json_encode(['success' => true, 'message' => 'Actividad ejecutada y reportada exitosamente']);
        } else {
            echo json_encode(['success' => false, 'message' => 'Error al ejecutar la actividad']);
        }
        exit();
    }
}
        ...
        <div class="calendario-controls">
            ...
            <div class="filters">
                ...
                <label style="margin-left: 20px;">
                    <input type="checkbox" id="filterMyEvents"> Mis Actividades
                </label>
            </div>
        </div>
